...
double measureSchedule(const std::vector<int>& vect1, const std::vector<int>& vect2, std::vector<int>& vect3,
    const std::string& scheduleType, int numThreads = 0, int chunkSize = 0) {
    // Set default number of threads
    if (numThreads == 0)
        numThreads = omp_get_num_procs();
...
int main() {
     ...
     * @section 1: Compare distribution of iterations amongst threads with low iteration count (e.g. 12)
     * - run with default chunk size (it does not default to 1, it splits the iterations into as evenly sized chunks as possible)
     * - run with specified chunk size (to seee round robin behavior)
     * @section 2: Compare execution time with increasing iteration count
     */
    ...
    const std::vector<std::string> kPerformanceHeaders = {"Size", "Static Total (s)", "Dynamic Total (s)", "Static Average (s)", "Dyanmic Average (s)"};
    ...
    std::cout << "\nPerformance Over Increasing Workloads\n" << kSingleLine << kSingleLine << std::endl;
    ...
    for (int i = kStartSize; i <= kMaxSize; i *= kSizeMultiplication) {
        ...
        for (int j = 0; j < kTestCount; j++) {
            ...
            staticTime += measureSchedule(vect1, vect2, vect3, "static", kNumThreads);
            dynamicTime += measureSchedule(vect1, vect2, vect3, "dynamic", kNumThreads);
        }
